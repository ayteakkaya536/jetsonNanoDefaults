...
def smooth(targetAngle, prevAngle,servoNumber):
    ...
    while abs(targetAngle - prevAngle) > 0.1:
        prevAngle = (targetAngle * 0.20) + (prevAngle * 0.80)
        outAngle = round(prevAngle,2)
        #    *** end of smoothing ***
        print(str(targetAngle) + " , " +str(outAngle))
...
while True:
    ...
    for (top,right,bottom,left),face_encoding in zip(facePositions,allEncodings):
        ...
        prevPanAngle, prevTiltAngle = servoTrack(left,top,right,bottom,width,height,prevPanAngle,prevTiltAngle)
